...
const Products = () => {
  ...
  return (
    ...
    <section id="products" className="py-16 section-gradient-3">
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-full font-medium transition-all ${
                categoriaActiva === categoria.id
                  ...
                  : 'bg-brand-box-2 text-brand-content hover:bg-gray-50 dark:hover:bg-neutral-700'
              }`}
            >
              {categoria.nombre}
            </button>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {productosFiltrados.map(producto => (
              <div 
                key={producto.id} 
                className="bg-brand-box-2 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 group h-full flex flex-col"
              >
                ...
                <div className="p-6 flex-grow flex flex-col">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="text-xl font-bold text-brand-titulo-logo line-clamp-1">
                      {producto.nombre}
                    </h3>
                    <span className="text-xs px-2 py-1 bg-gray-100 dark:bg-neutral-700 text-gray-600 dark:text-gray-300 border border-brand-border rounded-full whitespace-nowrap">
                      {producto.categoria}
                    </span>
                  </div>

                  <p className="text-brand-content text-sm mb-4 line-clamp-2 flex-grow">
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-brand-border">
                    <div>
                      <span className="text-2xl font-bold text-brand-subtitles-2">
